...
import SetQuantity from "../components/products/SetQantity";
...
const ItemContent: React.FC<ItemContentProps> = ({item}) => {
    ...
    return ( 
        ...
        md:text-sm gap-4 border-b-[1.5px] border-t-[1.5px] border-slate-200
        py-4 items-center">
            <div className="col-span-2 justify-self-start
            flex gap-2 md:gap-4">
                <Link href={`/product/${item.id}`}>
                    <div className="relative w-[70px] aspect-square">
                        <Image src={item.selectedImg.image} alt={item.name} fill className="object-contain"/></div>
                </Link>
